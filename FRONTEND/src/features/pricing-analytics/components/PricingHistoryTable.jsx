    ...
    <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-xs">
      <div className="flex items-center justify-between pb-3 border-b border-[#F1F5F9] mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-[#0F172A]">Pricing Revision Audit Log</h3>
            <p className="text-[11px] text-[#64748B]">
              Historical catalog adjustments and applied recommendations for {productName}
            </p>
          </div>
        </div>
      ...
      {isLoading ? (
        <div className="py-8 text-center text-xs text-[#64748B] animate-pulse">
          Loading pricing history records...
        </div>
      ) : error ? (
        ...
        <div className="py-8 text-center text-xs text-[#64748B] bg-[#F8FAFC] rounded-xl border border-dashed border-[#CBD5E1]">
          ...
          <p className="font-semibold text-[#0F172A]">No pricing history is available for this product.</p>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-[#F8FAFC] text-[#64748B] font-semibold border-b border-[#E2E8F0]">
              <tr>
                ...
                <th className="py-2.5 px-3">Recommendation ID</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#F1F5F9]">
              {history.map((record, index) => {
                ...
                const source =
                  ...
                  (record.recommendation_id ? 'AI Recommendation' : 'Manual Catalog Update');
                const status = (record.status || 'ACTIVE').toUpperCase();
                const recId = record.recommendation_id ? String(record.recommendation_id) : '—';
                ...
                return (
                  <tr key={record.id || index} className="hover:bg-[#F8FAFC] transition-colors">
                    ...
                    <td className="py-2.5 px-3 font-semibold font-mono text-[#0F172A]">
                      {price !== null && price !== undefined ? formatCurrency(price, currency) : '—'}
                    </td>
                    <td className="py-2.5 px-3 text-[#64748B] capitalize">{source}</td>
                    ...
                    <td className="py-2.5 px-3 font-mono text-[11px] text-[#64748B]">{recId}</td>
                  </tr>
                );
              })}
            </tbody>
